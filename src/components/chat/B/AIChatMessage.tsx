...
export const AIChatMessage = ({ from, children }: IAIChatMessageProps) => {
  if (from === 'user') {
    return (
      <div className='flex justify-end gap-2 my-2 first-of-type:mt-0 last-of-type:mb-0'>
        ...
        <div className='h-9 flex items-center text-orange'>
  ...
  return (
    <div className='flex justify-start gap-2 my-2 first-of-type:mt-0 last-of-type:mb-0'>
      <div className='h-9 flex items-center text-orange'>
        <Icon icon='fluent-mdl2:chat-bot' className='text-2xl' />
      </div>
      <div className='chat-message chat-message-assistant bg-orange before:border-r-orange text-white'>
        <div className='whitespace-pre-wrap break-words overflow-wrap break-word min-w-0'>
          {children}
        </div>
      </div>
